...
{
    public partial class frmApiPhpSoins : Form
    {
        public frmApiPhpSoins()
        {
            InitializeComponent();
        ...
        public void LoadSoinsInDataGridViewPHP()
        {
            try
            {
                var soins = servGetListSoinsWithPhp();
                if (soins != null && soins.Count > 0)
                {
                    DataTable dt = new DataTable();
                    dt.Columns.Add("ID Soin", typeof(int));
                    ...
                    // Style Guna2 vert
                    dtSoinsPHP.ColumnHeadersVisible = true;
                    dtSoinsPHP.EnableHeadersVisualStyles = false;
                    dtSoinsPHP.ColumnHeadersDefaultCellStyle.BackColor = Color.FromArgb(76, 175, 80); // Vert
                    dtSoinsPHP.ColumnHeadersDefaultCellStyle.ForeColor = Color.White;
                    dtSoinsPHP.ColumnHeadersDefaultCellStyle.Font = new Font("Segoe UI", 10, FontStyle.Bold);
                    dtSoinsPHP.AutoResizeColumnHeadersHeight();
                    dtSoinsPHP.AutoSizeColumnsMode = DataGridViewAutoSizeColumnsMode.Fill;
                    dtSoinsPHP.SelectionMode = DataGridViewSelectionMode.FullRowSelect;
                    dtSoinsPHP.ReadOnly = true;
                }
                else
                {
                    MessageBox.Show("Aucun soin trouvé ou erreur lors du chargement des données.", "Information", MessageBoxButtons.OK, MessageBoxIcon.Information);
